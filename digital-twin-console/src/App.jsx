...
function Thread({ dashboard }) {
  ...
  return <section className="view thread-view">
    ...
    {items.length ? <div className="timeline">{items.map((event,index)=><article key={event.eventId ?? index}><span className="timeline-node"/><div className="timeline-rev">R{event.revisionAfter ?? index+1}</div><div><strong>Event type {event.eventType ?? '—'}</strong><p>{event.actorDid || 'Actor DID unavailable'}</p><code>{shortId(event.eventId)}</code></div><time>{formatChainTime(event.createdAt)}</time></article>)}</div> : <Empty title="Digital Thread not indexed" text={dashboard?.thread?.error || "The Twin exists; events will appear when the indexer exposes the sequence."} />}
