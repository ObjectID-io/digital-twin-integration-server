import { ENERGY_STYLE, ENERGY_PANEL, ENERGY_CONTROLS, ENERGY_SCRIPT } from './energy-panel.js';
import { createServer } from "node:http";
import { createHash, timingSafeEqual } from "node:crypto";
import { SCENARIOS } from "./telemetry.js";
import { ENERGY_SCENARIOS, simulationSettings } from './energy.js';
...
export function createControlServer({ status, control, port, publishNow, recordTransition = async () => undefined, getStatus, controlTwin, adminPassword = "", installIntegrationConfig, removeIntegrationConfig }) {
  ...
  const server = createServer(async (request, response) => {
    ...
    const credentialReplacement = request.method === "PUT" && /^\/api\/integration\/(0x[0-9a-f]{64})$/i.exec(new URL(request.url, "http://simulator.local").pathname);
    if (credentialReplacement) {
      let value;
      ...
      const selectedTwinId = credentialReplacement[1].toLowerCase();
      if (!isHostedDeviceProvisioning(value)) return json(response, 400, { error: "Select a valid ObjectID Twin device configuration file" });
      if (String(value.twin.id).toLowerCase() !== selectedTwinId) return json(response, 409, { error: "The uploaded configuration belongs to a different Twin" });
      if (!installIntegrationConfig) return json(response, 503, { error: "Credential installation is unavailable" });
      try {
        const installed = await installIntegrationConfig(value, { verifyCredential: true });
        return json(response, 200, { updated: true, restarting: false, ...installed });
      } catch (error) {
...
export function applyCommand(command, control) {
  ...
  if (command?.action === 'profile') {
    const settings = simulationSettings({ profile: command.profile, energy: command.energy ?? control.energy });
    Object.assign(control, settings, { scenario: 'normal', energyStep: 0 });
    if (settings.profile === 'energy') control.mobileEnabled = false;
  }
  else if (command?.action === "pause") control.paused = true;
  else if (command?.action === "resume") control.paused = false;
  else if (command?.action === "toggle-mobility" && control.profile !== 'energy') control.mobileEnabled = !control.mobileEnabled;
  else if (command?.action === "enable-mobility" && control.profile !== 'energy') control.mobileEnabled = true;
  else if (command?.action === "disable-mobility") control.mobileEnabled = false;
  else if (command?.action === "reset") { control.paused = false; control.scenario = "normal"; }
  else if (command?.action === "scenario" && (control.profile === 'energy' ? ENERGY_SCENARIOS : SCENARIOS).includes(command.scenario)) control.scenario = command.scenario;
...
export function isHostedDeviceProvisioning(value) {
  if (value?.specVersion === "objectid.device-onboarding.v1") {
    try {
      onboardingConfig(value);
      const endpoint = new URL(value.mqtt.endpoint);
      return endpoint.protocol === "wss:" && endpoint.hostname === "dtis.objectid.io" && !endpoint.username && !endpoint.password && !endpoint.port && !endpoint.search && !endpoint.hash && endpoint.pathname === (value.network === "mainnet" ? "/mqtt-mainnet" : "/mqtt");
    } catch { return false; }
  }
...
function publicStatus(status, control) {
  return { ...status, profile: control.profile ?? 'machine', energy: control.energy, scenario: control.scenario, paused: control.paused, mobile: Boolean(control.mobileEnabled), changedAt: control.changedAt };
}
