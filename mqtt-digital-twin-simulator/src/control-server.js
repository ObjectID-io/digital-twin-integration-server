...
export function applyCommand(command, control) {
  ...
  else if (command?.action === "toggle-mobility") control.mobileEnabled = !control.mobileEnabled;
  else if (command?.action === "enable-mobility") control.mobileEnabled = true;
...
function publicStatus(status, control) {
  return { ...status, scenario: control.scenario, paused: control.paused, mobile: Boolean(control.mobileEnabled), changedAt: control.changedAt };
}
